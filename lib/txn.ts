...
// ── 은행/카드 양식별 프로파일 ─────────────────────────────
// 국내 은행/카드 명세서는 양식이 제각각이라, 양식 시그니처가 감지되면
// 해당 양식 전용 파서로 정확히 추출하고, 미감지 시 일반 파서로 폴백한다.
const normLine = (l: string) => l.replace(/\t/g, " ").replace(/\s+/g, " ").trim();
const inferYear = (t: string) => {
  const m = t.match(/(20\d{2})[.\-/]\s?\d{1,2}[.\-/]\d{1,2}/) || t.match(/\b(20\d{2})\d{4}\b/);
  return m ? m[1] : String(new Date().getFullYear());
};
const isMoneyTok = (s: string) => /\d/.test(s) && /^[-▲△(]?\d[\d,]*\)?-?$/.test(s);

interface BankProfile {
  id: string;
  source: StmtSource;
  detect: (text: string) => boolean;
  parse: (lines: string[], defYear: string) => Txn[];
}

// 카카오뱅크: 거래일시 구분(출금/입금) 거래금액(부호) 거래후잔액 거래내용 거래구분
function parseKakao(lines: string[]): Txn[] {
  const out: Txn[] = [];
  const re = /^(\d{4})\.(\d{1,2})\.(\d{1,2})\s+\d{1,2}:\d{2}(?::\d{2})?\s+(출금|입금)\s+(-?[\d,]+)\s+(-?[\d,]+)\s+(.+)$/;
  const kindRe = /(일반이체|일반입금|체크카드결제|이체입금|이체출금|자동이체|ATM출금|ATM입금|해외결제|간편결제|펌뱅킹|전자금융|이자|급여)\s*$/;
  for (const l of lines) {
    const m = l.match(re);
    if (!m) continue;
    const date = `${m[1]}-${m[2].padStart(2, "0")}-${m[3].padStart(2, "0")}`;
    const amt = (m[4] === "출금" ? -1 : 1) * Math.abs(num(m[5]));
    if (amt === 0) continue;
    const rest = m[7].trim();
    const km = rest.match(kindRe);
    const desc = km ? km[1] : "";
    const counterpart = (km ? rest.slice(0, km.index).trim() : rest).slice(0, 30);
    out.push({ date, amount: amt, desc: desc || counterpart || "거래", counterpart, source: "bank" });
  }
  return out;
}

// 중소기업은행: 거래일자 거래시간 [출금액][입금액] 잔액 거래점 거래내용 상대예금주 …
// 빈 칸(출금/입금)이 사라지므로 잔액 증감으로 부호를 복원한다.
function parseIBK(lines: string[]): Txn[] {
  const rows: { date: string; bal: number; mag: number; counterpart: string; desc: string }[] = [];
  for (const l of lines) {
    const m = l.match(/^(\d{4})-(\d{1,2})-(\d{1,2})\s+\d{1,2}:\d{2}:\d{2}\s+(.+)$/);
    if (!m) continue;
    const date = `${m[1]}-${m[2].padStart(2, "0")}-${m[3].padStart(2, "0")}`;
    const toks = m[4].split(" ").filter(Boolean);
    const lead: number[] = [];
    let i = 0;
    for (; i < toks.length; i++) {
      if (isMoneyTok(toks[i])) lead.push(Math.abs(num(toks[i])));
      else break;
    }
    if (lead.length < 1) continue;
    const bal = lead[lead.length - 1];
    const mag = lead.length >= 2 ? lead[lead.length - 2] : 0;
    if (mag === 0) continue;
    const textToks = toks.slice(i);
    const counterpart =
      textToks.find((t) => /[가-힣（）()]/.test(t) && !/은행|지점|영업점|뱅킹|점$/.test(t)) ||
      textToks[1] || textToks[0] || "";
    const desc = textToks[1] || textToks[0] || "";
    rows.push({ date, bal, mag, counterpart: counterpart.slice(0, 30), desc: desc.slice(0, 30) });
  }
  const out: Txn[] = [];
  for (let k = 0; k < rows.length; k++) {
    ...
    let sign = 0;
    if (prev && Math.abs(Math.abs(r.bal - prev.bal) - r.mag) < 2 && r.bal !== prev.bal) sign = Math.sign(r.bal - prev.bal);
    else if (next && Math.abs(Math.abs(r.bal - next.bal) - r.mag) < 2 && r.bal !== next.bal) sign = Math.sign(r.bal - next.bal);
    if (sign === 0) sign = -1; // 판별 불가 시 출금으로
    out.push({ date: r.date, amount: sign * r.mag, desc: r.desc || r.counterpart, counterpart: r.counterpart, source: "bank" });
  }
  return out;
}

// 삼성카드 세부 이용내역: … 이용일 가맹점명 이용금액(원) 개월수 승인번호
// (parseStatement 내부에서 천단위 콤마가 이미 제거되므로 콤마에 의존하지 않는다)
function parseSamsungCard(lines: string[]): Txn[] {
  const out: Txn[] = [];
  for (const l of lines) {
    if (/~|합계|이용기간|페이지|Page/i.test(l)) continue;
    const toks = l.split(" ").filter(Boolean);
    const di = toks.findIndex((t) => /^20\d{2}[-.]\d{1,2}[-.]\d{1,2}$/.test(t));
    if (di < 0 || toks.length < di + 4) continue;
    // 끝 3토큰 = 이용금액 / 개월수 / 승인번호
    const amtTok = toks[toks.length - 3];
    if (!/^\d[\d,]*$/.test(amtTok)) continue;
    const amount = -Math.abs(num(amtTok));
    if (amount === 0) continue;
    const merchant = toks.slice(di + 1, toks.length - 3).join(" ").slice(0, 30);
    const dm = toks[di].match(/(20\d{2})[-.](\d{1,2})[-.](\d{1,2})/)!;
    const date = `${dm[1]}-${dm[2].padStart(2, "0")}-${dm[3].padStart(2, "0")}`;
    out.push({ date, amount, desc: merchant || "카드이용", counterpart: merchant, source: "card" });
  }
  return out;
}

// 감지 시그니처는 추출 시 제목 글자가 띄어질 수 있어 헤더 키워드 기반으로 넓게 잡는다.
const BANK_PROFILES: BankProfile[] = [
  { id: "kakao", source: "bank", detect: (t) => /거래일시\s*구분\s*거래금액|카카오\s*뱅크/.test(t), parse: parseKakao },
  { id: "ibk", source: "bank", detect: (t) => /계좌별\s*거래명세표|거래일자\s*거래시간|출금액\s*입금액\s*잔액/.test(t), parse: parseIBK },
  { id: "samsung", source: "card", detect: (t) => /삼성\s*카드|가맹점명\s*이용금액|세부\s*이용내역|승인번/.test(t), parse: parseSamsungCard },
];
...
export function parseStatement(rawInput: string, source: StmtSource): Txn[] {
  ...
  // 양식 프로파일 우선 (감지된 것 중 결과가 나오는 첫 프로파일 사용)
  const normLines = rows.map(normLine).filter(Boolean);
  const fullText = normLines.join("\n");
  const year = inferYear(fullText);
  for (const p of BANK_PROFILES) {
    if (p.source !== source || !p.detect(fullText)) continue;
    const r = p.parse(normLines, year);
